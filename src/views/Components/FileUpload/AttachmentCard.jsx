...
const AttachmentCard = ({
  ...
  const [dltLoadingStates, setDltLoadingStates] = useState(initialLoadingStates);
  ...
  const fileDrop = (e) => {
    ...
    const { validFiles, errors } = filterFiles(newFiles, allowedFiles);
    setErrors(errors);
          ...
          <Box paddingX={5} mt={3} onDragOver={dragOver} onDrop={fileDrop}>
           ...
           {errors.length > 0 && 
            <Box sx={{position:'relative'}}>
              <IconButton sx={{position:'absolute' , top: 0 , right:0}} onClick={()=> setErrors('')}>
                <CloseIcon/>
              </IconButton>

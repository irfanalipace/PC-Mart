...
function AttachFileMenu({ openAttachmentMenu, setopenAttachmentMenu, onFileListUpdate }) {
    ...
    return (
        < >
            <Box
                sx={openAttachmentMenu === false ?
                    { display: 'none' } :
                    { display: 'block', position: 'absolute', background: 'white', boxShadow: 3, maxWidth: '365px' }
                }
            >
                <Box sx={{ textAlign: 'right', padding: '5px' }}>
                    <Close onClick={() => setopenAttachmentMenu(false)} sx={{ cursor: 'pointer' }} />
                </Box>
                <Box p={2}>

  ...
  const decline = async () => {
	...
	const downloadingPdf = async () => {
      ...
      const decId = decryptId(id)
			const resp = await downloadCustomerPriceQuoteApi({ id: decId });
	...
	const info = [
		{ label: 'Price Quote Ref:', value: priceQuote?.estimate_number || '' },
		{
			...
			value:
				priceQuote?.estimate_date &&
				formatDate(priceQuote?.estimate_date),
		},
		{
			label: 'Validity:',
			value:
				priceQuote?.expiry_date &&
				formatDate(priceQuote?.expiry_date),
		},
	];

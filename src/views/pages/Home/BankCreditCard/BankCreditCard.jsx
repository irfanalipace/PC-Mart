...
export default function BankCreditCard(props) {
  ...
  return (
    <Box>
      <Card variant="outlined" sx={{ width: '520px', height: '548px', marginTop: '44px' }}>
        <CardContent>
          <Typography variant="h6">{title}  <HoverPopover text="Bank and Credit Cards">
        <HelpOutlineIcon sx={{ color: 'gray', fontSize: '12px' }} />
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: '200px' }}>
            <Typography variant="h6" sx={{ textAlign: 'center' }}>{bankDetalis}</Typography>
            <Button sx={{ color: "#00afdced", padding: '0px', border: '0px' }}>{addBankAccount}</Button>
          </Box>
        </CardContent>
      </Card>
    </Box>

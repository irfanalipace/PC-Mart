...
const NewCreditMemo = ({ edit }) => {
	...
	// 	// const customerId = decryptedID(params.get('customerId'));
	// 	if (decryptedID && customerOptions.length > 0) {
	// 		const defaultVal = customerOptions?.find(item => item?.id == decryptedID);
	// 		setSelectedCustomer({
	// 			label: defaultVal?.display_name,
	// 			value: defaultVal?.id,
	// 		});
	// 		formik.setFieldValue('customer_id', decryptedID);
	// 	}
	...
	const deletingFile = fileId => {
		setCreditMemoFiles(prevFiles => prevFiles.filter(f => f !== fileId));
	};
	...
	useEffect(() => {
		...
		if (customer) {
			setInvoiceList(customer?.invoices);
			setSelectedCustomer({
				...
				>
						...
						<Grid item container>
							...
							<Grid item xs={4}>
								<CustomSelect
									...
									onChange={selected => {
										formik.setFieldValue('customer_id', selected.value);
										setSelectedCustomer({
                  ...
                  <SearchIcon />
                        ...
                        Attach file(s) to Credit Memo
											...
											<List>
												{creditMemoFiles?.map((file, index) => (
													<ListItem key={index}>
														<Typography>

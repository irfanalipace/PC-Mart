...
import useResponsiveStyles from 'invoicing/src/hooks/useMedaiQuery';
...
const AddItemBill = React.memo(
  ({
    ...
    const determineInitialValues = () => {
      ...
      if (initialValues?.invoice_items) return initialValues.invoice_items;
    ...
    useEffect(() => {
      ...
      if (isEdit) {
        var updatedItems = determineInitialValues().map((item, index) => {
    ...
    const handleDeleteRow = (rowId, index) => {
      ...
      } else if (initialValues.invoice_items) {
        const updatedItems = [...formik.values.invoice_items];
        updatedItems.splice(index, 1);
        formik.setFieldValue('invoice_items', invoice_items);
      }
    };
    ...
    useEffect(() => {
      if (clicked === true) {
        ...
        //   filteredItems,
        // );
        const conditionMetBeyondIndex0 = filteredItems.slice(1).some((item) => {
          return (
            item.item_name !== 'Type or click to select an item' ||
            item.amount !== 0
          );
        ...
        // If the condition is met beyond index 0, remove the item at index 0
        // if (conditionMetBeyondIndex0) {
        //   filteredItems.shift(); // Removes the item at index 0
        // }
    ...
    useEffect(() => {
      if (submitCount > 0 && isValid) {
        const valuesss = Object.entries(labelValueMap).map(
          ([label, value], index) => {
            const taxInclusiveAmount = labelToTaxAmountMap[label]?.toFixed(2);
            ...
            if (
              ...
              const result = ((accumulatedAmount * value) / 100)?.toFixed(2);
        ...
        const updateFormFields = () => {
          ...
          const shippingChargesResult = isGreaterThanZero(shippingCharges)
            ? shippingCharges?.toFixed(2)
            : '0.00';
          formik.setFieldValue(`shipping_charges`, shippingChargesResult);

          // Calculate Adjustment
          const adjustmentResult = isGreaterThanZero(adjustmentValue)
          ...
          if (initialValues.estimate_items) {
            filteredItems.forEach((item, index) => {
              ...
              formik.setFieldValue('tax_amount', sum);
            ...
            filteredItems.forEach((item, index) => {
              ...
              const updateFieldValues = () => {
                ...
                if (
                  ...
                  var txid = 1;
                  formik.setFieldValue(`invoice_items.${index}.tax_id`, txid);
                } else {
                  formik.setFieldValue(`invoice_items.${index}.tax_id`, tax_id);
                }
              };
    ...
    useEffect(() => {
      ...
      resp
        .then((data) => {
          ...
          const updatedOptions = reps.map(({ id, name, item_sale }) => ({
            value: id,
            label: name,
    ...
    return (
      <>
        <Grid item container>
          ...
          <Grid item xs={2} >
            <AddRowTitle sx={{marginLeft:'20px'}}>Account</AddRowTitle>
          </Grid>
            ...
            values.map((row, index) => (
              <Grid container columnSpacing={1} key={index}>
                ...
                <Grid item xs={2}>
                  <CreatableSelect
                    ...
                    onChange={(newValue) => {
                      ...
                      const newRate = selectedOption ? selectedOption.price : 0;
                      setValues((prevValues) => {
                ...
                <Grid item xs={1} display='flex' justifyContent='flex-start'>
                  ...
                  $ {isNaN(values[index].amount) ? '0' : values[index].amount}
                  </InputLabel>
                </Grid>
          ...
          <Grid item xs={12} my={2} mt={0}>
            <MUIButton onClick={handleAddRow} endIcon={<AddIcon /> }> 
              Add row
            </MUIButton>
          </Grid>
        </Grid>
              ...
              <Grid container>
                ...
                >
                  ...
                  <Grid item xs={4} display='flex' justifyContent='flex-end'>
                    ...
                    $ {isNaN(calculateSubtotal()?.toFixed(2))
                        ? '0.00'
                  ...
                  <Grid item xs={4} display='flex' justifyContent='flex-end'>
                    ...
                    $ {selectedOption === 'Percentage'
                        ? isNaN((calculateSubtotal() * numberChange) / 100)
                          ? '0.00'
                          : `-${(
                              (calculateSubtotal() * numberChange) /
                              100
                            )?.toFixed(2)}`
                ...
                >
                  ...
                  $  {isNaN(calculateTotal()?.toFixed(2))
                      ? '0.00'
